      ...
      <div className="edit-wrap">
        ...
        <div className="edit-card">
          <div className="form-grid">
            ...
            <input className={errors.name ? 'err' : ''} aria-invalid={!!errors.name} value={form.name} onChange={set('name')} />
            {errors.name && <span className="help-err">{errors.name}</span>}

            <label>Цена</label>
            <input className={errors.price ? 'err' : ''} aria-invalid={!!errors.price} type="number" step="0.01" min="0" value={form.price} onChange={set('price')} />
            {errors.price && <span className="help-err">{errors.price}</span>}

            <label>Тип</label>
            <select className={errors.type ? 'err' : ''} aria-invalid={!!errors.type} value={form.type} onChange={set('type')}>
            ...
            <label>Кол-во</label>
            <input className={errors.number ? 'err' : ''} aria-invalid={!!errors.number} type="number" min="0" value={form.number} onChange={set('number')} />
            {errors.number && <span className="help-err">{errors.number}</span>}

            <label>Скидка (0–100)</label>
            <input className={errors.discount ? 'err' : ''} aria-invalid={!!errors.discount} type="number" min="0" max="100" step="0.01" value={form.discount} onChange={set('discount')} />
            ...
            <label>X</label>
            <input className={errors.coordX ? 'err' : ''} aria-invalid={!!errors.coordX} type="number" step="1" value={form.coordX} onChange={set('coordX')} />
            {errors.coordX && <span className="help-err">{errors.coordX}</span>}

            <label>Y</label>
            <input className={errors.coordY ? 'err' : ''} aria-invalid={!!errors.coordY} type="number" step="0.01" value={form.coordY} onChange={set('coordY')} />
            {errors.coordY && <span className="help-err">{errors.coordY}</span>}

            <div className="fieldset">Связанные сущности</div>

            <label>Person</label>
            <select className={errors.personId ? 'err' : ''} aria-invalid={!!errors.personId} value={form.personId} onChange={set('personId')}>
              <option value="">— не задан —</option>
              {persons.length > 0 &&
                  persons.map((p) => (
            ...
            <label>Event</label>
            <select className={errors.eventId ? 'err' : ''} aria-invalid={!!errors.eventId} value={form.eventId} onChange={set('eventId')}>
            ...
            <label>Venue</label>
            <select className={errors.venueId ? 'err' : ''} aria-invalid={!!errors.venueId} value={form.venueId} onChange={set('venueId')}>
              <option value="">— не задано —</option>
              {venues.length > 0 &&
                  venues.map((v) => (
